...
for cbname in `find cookbooks -maxdepth 1 -mindepth 1 -type d | sed -e 's/cookbooks\///'`; do
  ...
  foodcritic --tags ~FC001 --tags ~FC020 --tags ~FC035 $@ cookbooks/$cbname 2>&1 | ./chef-ci-tools/bin/foodcritic2junit.pl --suite $cbname --out junit_reports/foodcritic-$cbname.xml
done
